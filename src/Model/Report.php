...
class Report
{
	...
	public function redeemedByCID($cid){
		...
		$sql = "SELECT title as campaign, campaign as id, SUM(vouchers.rate) as total, redeem_date, vouchers.redeem_id as receipt from vouchers JOIN campaigns on vouchers.campaign= campaigns.id where redeem != 0 and campaign = :cid group by redeem_id";
	...
	public function redeemedByReceipt( $cid){
		...
		$sql = "SELECT title, campaign,  SUM(vouchers.rate) as total, vouchers.redeem_date, vouchers.redeem_id, user_name, user_phone, user_comment from vouchers JOIN campaigns on vouchers.campaign= campaigns.id 
join receipt on receipt.redeem_id = vouchers.redeem_id
where redeem != 0 and campaign =:cid group by redeem_id";
		$stmt = $this->conn->prepare( $sql );
		$stmt->bindParam( ":cid", $cid, PDO::PARAM_INT );
		$stmt->execute();
		$result = $stmt->fetchAll(PDO::FETCH_ASSOC);
